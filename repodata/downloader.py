...
def forgeInstallerDownloader(downloadToDir, manifestPath):
    ...
    try:
        ...
        forgeData = requests.get('https://maven.minecraftforge.net/net/minecraftforge/forge/' + minecraftVersion +
                                 '-' + forgeVersion + '/forge-' + minecraftVersion + '-' + forgeVersion + '-installer.jar')
    except:
...
def modDownloader(downloadToDir, manifestPath, exceptManifestPath=None):
    ...
    for file in manifest['files']:
        # get mod information
        try:
            modInformation = requests.get('https://addons-ecs.forgesvc.net/api/v2/addon/' + str(
                file['projectID']) + '/file/' + str(file['fileID']) + '/', headers=headers).json()
        except:
            print('cant get information for projectID: ' +
                  str(file['projectID']) + ' with fileID: ' + str(file['fileID']))
            exit()
        ...
        if modData.status_code != 200:
            print('cant download ' + modInformation["fileName"] + ' from ' +
                  modInformation["downloadUrl"] + ' because of status code ' + str(modData.status_code))
            exit()

        # save mod data
        try:
            if file['required'] == True:
                f = open(os.path.join(downloadToDir,
                         modInformation["fileName"]), 'wb')
            else:
                f = open(os.path.join(downloadToDir,
                         modInformation["fileName"] + '.disabled'), 'wb')
